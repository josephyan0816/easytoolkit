//
// Created by Joseph Yan on 2023/1/15.
//

#ifndef EASYTOOLKIT_RESOURCEPOOL_H
#define EASYTOOLKIT_RESOURCEPOOL_H
...
#define SUPPORT_DYNAMIC_TEMPLATE
    ...
    template<typename C>
    class share_ptr_imp : public std::shared_ptr<C> {
        /**
         * @param 构造智能指针
         * @param weakPool 管理本指针的循环池
         * @param quit 对接是否放弃循环使用
         */
        share_ptr_imp(C *ptr, const std::weak_ptr<ResourcePool_l<C>> &wealPool, std::shared_ptr<std::atomic_bool> quit,
    ...
    public:
        using ValuePtr = share_ptr_imp<C>;

        friend class share_ptr_imp<C>;
        ...
        void recycle(C *obj) {
            ...
            if (!is_busy) {
                ...
                if (_objs.size() >= _pool_size) {
                    ...
                    _objs.template emplace_back(obj);
                }
                _busy.clear();
            } else {
                delete obj;
    ...
    template<typename C>
    share_ptr_imp<C>::share_ptr_imp(C *ptr, const std::weak_ptr<ResourcePool_l<C>> &wealPool,
                                    std::shared_ptr<std::atomic_bool> quit,
                                    const std::function<void(C *)> &on_recycle):std::shared_ptr<C>(ptr,[wealPool,quit,on_recycle](C *ptr) {
        if(on_recycle){
            on_recycle(ptr);
        }
        auto strongPool=wealPool.lock();
        if(strongPool&&!(*quit)){
            //循环池还在并且不放弃循环放入循环池
            strongPool->recycle(ptr);
        }else{
...
#endif //EASYTOOLKIT_RESOURCEPOOL_H
