    ...
    void LogChannel::format(const Logger &logger, std::ostream &ost, const LogContextPtr &ctx, bool enable_color,
        ...
        if (enable_detail) {
            ...
            ost << "[" << getpid() << "-" << ctx->_thread_name << "] ["
                << (!ctx->_flag.empty() ? ctx->_flag : logger.getName());
#endif
...
///////////////////FileChannelBase///////////////////
    FileChannelBase::FileChannelBase(const std::string &name, const std::string &path, LogLevel level) : LogChannel(
            name, level), _path(path) {
    ...
    FileChannel::FileChannel(const std::string &name, const std::string &dir, LogLevel level) : FileChannelBase(name,
                                                                                                                "",
                                                                                                                level) {
        _dir = dir;
        if (_dir.back() != '/') {
