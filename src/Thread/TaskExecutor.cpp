...
namespace toolkit {
    ThreadLoadCounter::ThreadLoadCounter(uint64_t max_size, uint64_t max_usec) {
        _last_sleep_time = _last_wake_time = getCurrentMillisecond();
        _max_size = max_size;
        _max_usec = max_usec;
    }
    ...
    void ThreadLoadCounter::startSleep() {
        ...
        _sleeping = true;
        auto current_time = getCurrentMillisecond();
    ...
    int ThreadLoadCounter::load() {
        ...
        if (_sleeping) {
            totalSleepTime += (getCurrentMillisecond() - _last_sleep_time);
        } else {
            totalRunTime += (getCurrentMillisecond() - _last_wake_time);
        }
...
////////////////////////////////////////////////////////////////////////////
    ...
    void TaskExecutorGetterImp::getExecutorDelay(const std::function<void(const std::vector<int> &)> &callback) {
    ...
    void TaskExecutorGetterImp::for_each(const std::function<void(const TaskExecutor::Ptr &)> &cb) {
        for (auto &th: _threads) {
            cb(th);
        }
    }
    ...
    size_t TaskExecutorGetterImp::addPoller(const std::string &name, size_t size, int priority, bool register_thread,
